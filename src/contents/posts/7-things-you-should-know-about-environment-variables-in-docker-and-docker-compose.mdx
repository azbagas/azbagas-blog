...
<Alert type="note">
`EXPOSE` in a `Dockerfile` is **documentation only**. It signals to users of the image that the container uses that port, but it does not automatically publish it to the host. The port is only truly accessible from the host when you use the `-p` flag with `docker run` or the `ports:` attribute in docker-compose.
</Alert>
